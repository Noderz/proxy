...
var fs = require('fs'),
	...
	rewriter = require(path.join(__dirname, 'rewrite.js')),
	server = new nodehttp.server({
		port: config.port,
		address: config.address,
		static: path.join(__dirname, 'public'),
		ssl: config.ssl ? { key: fs.readFileSync(path.join(__dirname, 'ssl.key'), 'utf8'), cert: fs.readFileSync(path.join(__dirname, 'ssl.crt'), 'utf8') } : false,
	}),
	rw = new rewriter({
		prefix: '/service',
